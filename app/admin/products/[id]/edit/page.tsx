...
export default function EditProductPage() {
      ...
      try {
        ...
        if (product) {
          ...
          if (product.gallery) {
            ...
            if (dims.length < 5 && dims.length > 0 && dims[dims.length - 1] !== null) {
              dims.push(null);
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Keep existing images (those with URLs but no new File)
      galleryPreviews.forEach((url, i) => {
        if (url && !galleryImageFiles[i]) {
